(ns marketentry.marketentryllm
  ...
  the Trinidad and Tobago (TTO) public-sector market-entry compliance
  actor.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world oprtt.org Procurement
  Depository submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "operator-company-type=" (:operator-company-type e)
                        " foreign-investor?=" (:foreign-investor? e)
                        " foreign-shareholding-pct=" (:foreign-shareholding-pct e)
                        " bir-number-verified?=" (:bir-number-verified? e)
     ...
     :confidence (if (and e (:bir-number-verified? e))
                   0.9 0.3)}))
