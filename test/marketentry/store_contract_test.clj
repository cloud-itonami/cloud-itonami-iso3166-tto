...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Ltd" :jurisdiction "TTO"
                                   ...
                                   :foreign-investment-notice-filed? nil
                                   :foreign-investor-licence-held? nil
                                   :bir-number-verified? true
                                   :drafted? false :submitted? false :status :intake}})
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "TTO" :checklist ["a"] :spec-basis "x"}})
