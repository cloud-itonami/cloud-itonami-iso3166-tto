...
(deftest clean-intake-auto-commits
  (let [[db actor] (fresh)
    ...
    (is (= "Piarco Logistics Ltd" (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest foreign-investment-noncompliant-private-path-is-held-and-unoverridable
  (testing "PRIVATE company, foreign investor, s.4 notice not filed -> HARD hold (flagship check, private path)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:foreign-investment-noncompliant} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest foreign-investment-noncompliant-public-path-is-held-and-unoverridable
  (testing "PUBLIC company, 45% cumulative foreign shareholding (>= 30% threshold), no licence held -> HARD hold (flagship check, public path)"
    (let [[db actor] (fresh)
...
(deftest foreign-investment-below-threshold-never-holds
  (testing "PUBLIC company at 20% foreign shareholding (BELOW the 30% s.5(2) threshold), no licence held -- must NOT hold, proving the threshold conditionality"
    (let [[_db actor] (fresh)
          _ (assess! actor "t7pre" "eng-7")
          _ (draft! actor "t7pre" "eng-7")
          res (exec-op actor "t7" {:op :filing/submit :subject "eng-7"} operator)]
      (is (= :interrupted (:status res)) "clean and settles into the actuation-approval interrupt, not a hold")
      (let [r2 (approve! actor "t7")]
        (is (= :commit (get-in r2 [:state :disposition])))))))

(deftest foreign-investment-private-notice-filed-never-holds
  (testing "PRIVATE foreign investor with s.4 notice ALREADY filed -- must NOT hold, proving the satisfied-path does not misfire"
    (let [[_db actor] (fresh)
          _ (assess! actor "t8pre" "eng-8")
          _ (draft! actor "t8pre" "eng-8")
          res (exec-op actor "t8" {:op :filing/submit :subject "eng-8"} operator)]
      (is (= :interrupted (:status res)) "clean and settles into the actuation-approval interrupt, not a hold")
...
(deftest engagement-fee-mismatch-is-held
  (testing "claimed fee that doesn't equal base + months x rate -> HOLD"
    (let [[db actor] (fresh)
          _ (assess! actor "t9pre" "eng-3")
          _ (draft! actor "t9pre" "eng-3")
          res (exec-op actor "t9" {:op :filing/submit :subject "eng-3"} operator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:engagement-fee-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest bir-number-unverified-is-held-and-unoverridable
  (testing "unverified BIR File Number -> HARD hold"
    (let [[db actor] (fresh)
          _ (assess! actor "t10pre" "eng-5")
          _ (draft! actor "t10pre" "eng-5")
          res (exec-op actor "t10" {:op :filing/submit :subject "eng-5"} operator)]
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:bir-number-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
